...
function AutoUpdate(data)
    if tonumber(data) > tonumber(Version) then
        ...
        DownloadFileAsync("https://raw.githubusercontent.com/xSxcSx/SL-Series/master/SL-Aggro.lua", COMMON_PATH .. "SL-Aggro.lua", function() print("Update Complete, please 2x F6!") return end)
    end
end
GetWebResultAsync("https://raw.githubusercontent.com/xSxcSx/SL-Series/master/SL-Aggro.version", AutoUpdate)

class 'Aggro'

function Aggro:__init(u,t,ta)
	self.table = ta or false
	self.unit = u 
	...
	Callback.Add("ProcessSpell", function(u,s) self:PS(u,s) end)
	Callback.Add("ProcessSpellComplete", function(u,s) self:PSC(u,s) end)
	Callback.Add("Tick", function() self:T() end)
end

function Aggro:PS(u,s)
	if u and s and s.target then 
		for _,i in pairs(self.u) do
			if s.target.networkID == _ then
				table.insert(self.a, {s=s,a=u,t=i.i})
				i.c = i.c+1
			end
		end
	end 
...
function Aggro:PSC(u,s)
	if u and s and s.target then 
		for _,i in pairs(self.a) do
			if i.a.networkID == u.networkID then
				table.remove(self.a, _)
				DelayAction(function()
					for k,p in pairs(self.u) do
						p.c = p.c-1
					end
				end,(os.clock() + GetDistance(s.target,u) / (self.ps[u.charName] or math.huge) - GetLatency()/2000)*.001)
			end
		end
	end
end
...
function Aggro:T()
	if self.table then
		for _,i in pairs(self.unit) do
			if i and i.alive and i.valid and not self.u[i.networkID] and i.team == self.team then
				self.u[i.networkID] = {c=0,i=i}
			end
			if i and i.dead and i.team == self.team then
				self.u[i.networkID] = nil
			end
		end
	else
		if self.unit and self.unit.alive and self.unit.valid and not self.u[self.unit.networkID] and self.unit.team == self.team then
			self.u[self.unit.networkID] = {c=0,i=self.unit}
		end
		if self.unit and self.unit.dead and self.unit.team == self.team then
			self.u[i.networkID] = nil
		end
	end
	for _,i in pairs(self.u) do
...
function Aggro:GetAttacker()
	for _,i in pairs(self.a) do
		return i.a
	end
end

function Aggro:GetTarget()
	for _,i in pairs(self.u) do
		for k,p in pairs(self.a) do
			if p.a then
				return i.i
			end
		end
